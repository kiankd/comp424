...
public class Coordinates {
	...
	public static List<Coord> getNeighbors(Coord c) {
		...
		for (int incr : Arrays.asList(-1, 1)) {
			try { neighbors.add(get(c.x + incr, c.y)); } catch (IndexOutOfBoundsException e) { }
			try { neighbors.add(get(c.x, c.y + incr)); } catch (IndexOutOfBoundsException e) { }
		}
		return neighbors;
	}
	...
	public static Coord getSandwichCoord(Coord front, Coord middle) throws CoordinateDoesNotExistException {
		...
		if (xDiff * yDiff != 0 || xDiff + yDiff == 0) {
			throw new CoordinateDoesNotExistException("These coordinates are not adjacent: " +
					front.toString() + " and " + middle.toString());
		}
		try {
