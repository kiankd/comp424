...
public class GreedyTablutPlayer extends TablutPlayer {
	...
	public GreedyTablutPlayer() { super("GreedyPlayer"); }
	public GreedyTablutPlayer(String name) { super(name); }
	...
	public Move chooseMove(TablutBoardState bs) {
		...
		for (TablutMove move : options) {
			...
			/* If we also want to check if the move would cause us to win, this works for both!
			 * This will check if black can capture the king, and will also check if white can
			 * move to a corner, since if either of these things happen then a winner will be set. */
		...
		if (player_id == TablutBoardState.SWEDE && !moveCaptures) {
			...
			for (TablutMove move : bs.getLegalMovesForPosition(kingPos)) {
				/* Here it is not necessary to actually process the move on a copied boardState. Note that
				 * it is more efficient NOT to copy the boardState. Consider this during implementation... */
